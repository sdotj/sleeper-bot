...
export class EspnClient implements EspnApi {
  private playersCache: EspnPlayer[] | null = null;
  ...
  private async get<T>(url: string, headers: Record<string, string> = {}): Promise<T> {
    const res = await fetch(url, { headers: this.headers(headers) });
    if (res.status === 401) {
      throw new Error(
  ...
  async getPlayers(): Promise<EspnPlayer[]> {
    ...
    // ownership-sorted slice (covers every fantasy-relevant player) once.
    ...
    this.playersCache = (league.players ?? []).map((p) => p.player).filter((p): p is EspnPlayer => !!p);
